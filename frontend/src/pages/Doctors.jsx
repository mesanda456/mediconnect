...
import { Plus, Trash2, Loader2, Pencil, X, UserCheck, Phone, Mail, Star, DollarSign, Search } from 'lucide-react';
import toast from 'react-hot-toast';
import API from '../api/axios';
import { exportDoctorsPDF } from '../utils/pdfExport';
...
function Doctors() {
  ...
  const filtered = doctors.filter(d => {
    ...
    const matchAvailable = filterAvailable === 'ALL' ||
      (filterAvailable === 'AVAILABLE' && d.available) ||
      (filterAvailable === 'UNAVAILABLE' && !d.available);
  ...
  const getAvatarColor = (name) => {
    const colors = ['from-emerald-400 to-emerald-600', 'from-blue-400 to-blue-600', 'from-purple-400 to-purple-600', 'from-orange-400 to-orange-600', 'from-teal-400 to-teal-600', 'from-pink-400 to-pink-600'];
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <button
            onClick={() => exportDoctorsPDF(filtered)}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 border-l-4 border-l-emerald-500">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2 flex gap-3 pt-2">
              <button type="submit" disabled={saving} className="flex items-center gap-2 bg-emerald-600 text-white px-6 py-2.5 rounded-xl hover:bg-emerald-700 disabled:opacity-50 font-medium transition-all">
                {saving && <Loader2 className="w-4 h-4 animate-spin" />}
                {saving ? 'Saving...' : editingId ? 'Update Doctor' : 'Save Doctor'}
              </button>
              <button type="button" onClick={handleCancel} className="bg-gray-100 text-gray-600 px-6 py-2.5 rounded-xl hover:bg-gray-200 font-medium transition-all">Cancel</button>
            </div>
          </form>
        </div>
      )}
      ...
      <div className="flex gap-3 flex-wrap">
        <div className="relative flex-1 min-w-48">
          ...
          <input
            className="pl-10 border border-gray-200 rounded-xl p-2.5 w-full focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-white"
        ...
        <select
          className="border border-gray-200 rounded-xl p-2.5 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm text-gray-600 bg-white"
        ...
        {(search || filterSpecialty !== 'ALL' || filterAvailable !== 'ALL') && (
          <button
            onClick={() => { setSearch(''); setFilterSpecialty('ALL'); setFilterAvailable('ALL'); }}
      ...
      <div className="flex gap-4 text-sm text-gray-500">
        <span>Showing <strong className="text-gray-700">{filtered.length}</strong> of <strong className="text-gray-700">{doctors.length}</strong> doctors</span>
        <span>•</span>
        <span className="text-emerald-600 font-medium">{doctors.filter(d => d.available).length} available</span>
        <span>•</span>
        <span className="text-red-400">{doctors.filter(d => !d.available).length} unavailable</span>
      </div>
            ...
            <div key={d.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-all">
              ...
              <div className="space-y-2 mb-4">
                {d.qualification && (
                  <div className="flex items-center gap-2 text-xs text-gray-500">
                    <Star className="w-3.5 h-3.5 text-amber-400" />{d.qualification}
                  </div>
                )}
                {d.email && (
                  <div className="flex items-center gap-2 text-xs text-gray-500">
                    <Mail className="w-3.5 h-3.5 text-gray-400" />{d.email}
                  </div>
                )}
                {d.phone && (
                  <div className="flex items-center gap-2 text-xs text-gray-500">
                    <Phone className="w-3.5 h-3.5 text-gray-400" />{d.phone}
                  </div>
                )}
              </div>
              <div className="flex items-center justify-between pt-4 border-t border-gray-50">
                <div className="flex items-center gap-1">
                  ...
                  {d.experienceYears && <span className="text-xs text-gray-400 ml-2">• {d.experienceYears} yrs</span>}
                </div>
                <div className="flex gap-1">
                  <button onClick={() => handleEdit(d)} className="p-2 hover:bg-blue-50 rounded-lg text-gray-400 hover:text-blue-600 transition-colors">
                    <Pencil className="w-4 h-4" />
                  </button>
                  <button onClick={() => handleDelete(d.id)} className="p-2 hover:bg-red-50 rounded-lg text-gray-400 hover:text-red-500 transition-colors">
